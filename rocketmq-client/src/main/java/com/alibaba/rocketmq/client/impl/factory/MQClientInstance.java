        ...
        if (map != null && !map.isEmpty()) {
            ...
            if (!found) {
                ...
                if (brokerAddrs.size() > 0) {
                    ...
                    //随机找一个Slave
                    int randonIndex = 0;
                    if (brokerAddrs.size() > 2) {
                        randonIndex = (new Random(new Date().getTime()).nextInt()) % brokerAddrs.size(); //随机找到一个Slave位置.
                    }


                    brokerAddr = brokerAddrs.get(randonIndex);
                    found = true;
                    slave = true;
                }
            }
