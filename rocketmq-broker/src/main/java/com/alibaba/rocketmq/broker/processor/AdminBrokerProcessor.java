...
public class AdminBrokerProcessor implements NettyRequestProcessor {
    ...
    public RemotingCommand processRequest(ChannelHandlerContext ctx, RemotingCommand request)
        ...
        switch (request.getCode()) {
             ...
             * by geewu 2015-1-7
             */
            case RequestCode.GET_PRODUCERGROUP_ACTIVE_LIST:
                return this.getProducerGroupList(ctx, request);
    ...
    private RemotingCommand getProducerGroupList(ChannelHandlerContext ctx, RemotingCommand request)
            throws RemotingCommandException {
        final RemotingCommand response = RemotingCommand.createResponseCommand(null);
        ...
        ProducerGroup bodydata = new ProducerGroup();

        HashMap<String, HashMap<Channel, ClientChannelInfo>> groupNames =
                this.brokerController.getProducerManager().queryProducerGroup(requestHeader.getProducerGroup());

        if (groupNames != null) {

            HashSet<ProducerGroupRunningInfo> runningInfos = new HashSet<ProducerGroupRunningInfo>();
            for (Map.Entry<String, HashMap<Channel, ClientChannelInfo>> group : groupNames.entrySet()) {

                ProducerGroupRunningInfo runningInfo = new ProducerGroupRunningInfo();
                runningInfo.setProducerGroup(group.getKey());
                runningInfo.setClientCount(group.getValue().size());

                runningInfos.add(runningInfo);
            }

            bodydata.setGroupRunningInfos(runningInfos);
